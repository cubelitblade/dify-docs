...
| `API_WEBSOCKET_WORKER_AMOUNT` | `1` | Docker Compose のみ。リアルタイムコラボレーションを担う専用 `api_websocket` コンテナの Gunicorn ワーカー数です。コラボレーションのイベントは Redis 経由でワーカー間に同期されます。編集トラフィックが多く、1 つのワーカーが CPU コアを使い切る場合は安心して増やせます。 |
...
| `API_WEBSOCKET_WORKER_CONNECTIONS` | `1000` | Docker Compose のみ。`api_websocket` ワーカー 1 つあたりが同時に受け付ける WebSocket 接続の最大数です。開いているワークフローエディタタブごとに 1 接続を使用するため、デフォルトではワーカー 1 つあたり最大 1000 の同時編集セッションを処理できます。それ以上が見込まれる場合は、値を増やすかワーカーを追加してください。 |
...
デフォルト値：`worker`（worker サービスの env ファイルに設定された値。変数が未設定の場合、コンテナは API サーバーを起動）

`dify-api` イメージがコンテナ起動時に実行するプロセスを選択します。3 つの env サンプルファイルがそれぞれのサービス向けの値を設定しています：`worker.env.example` は `worker`、`api.env.example` は `api`、`worker-beat.env.example` は `beat`。指定できる値：

- `api`（空または認識されない値も同様）：API サーバー
- `worker`：Celery タスクコンシューマー
- `beat`：定期タスクディスパッチャー
- `job`：Flask CLI コマンドを 1 つ実行して終了
- `migration`：`MIGRATION_ENABLED=true` と併用し、データベース移行後に終了

付属の Compose ファイルはサービスごとに適切な値を設定済みのため、変更が必要なのは Kubernetes などのカスタム構成のみです。誤った値ではコンテナが別のプロセスを実行します。worker がないと、インデックス作成やメールのタスクが処理されずに滞留します。
...
| `REDIS_KEEPALIVE` | `true` | Redis 接続で TCP keepalive を有効にします。ファイアウォールやロードバランサーに静かに切断されたアイドル接続を検知し、次回使用時に失敗する前に再確立します。 |
| `REDIS_KEEPALIVE_IDLE` | `30` | 最初の TCP keepalive プローブを送信するまでの接続アイドル秒数（`TCP_KEEPIDLE`）。`REDIS_KEEPALIVE=true` のときに適用されます。 |
| `REDIS_KEEPALIVE_INTERVAL` | `10` | TCP keepalive プローブの送信間隔（秒）（`TCP_KEEPINTVL`）。`REDIS_KEEPALIVE=true` のときに適用されます。 |
| `REDIS_KEEPALIVE_COUNT` | `10` | 接続が切断されたと判断するまでに許容する未確認プローブの回数（`TCP_KEEPCNT`）。`REDIS_KEEPALIVE=true` のときに適用されます。 |
...
### Dify CLI とプログラマティック API

`/openapi/v1/*` は、[Dify CLI](/ja/cli/quick-start)（`difyctl`）などのクライアントが OAuth ベアラートークンで利用する、ユーザー単位のプログラマティック API です。デフォルトでは無効です。CLI を使うには `OPENAPI_ENABLED` と `ENABLE_OAUTH_BEARER` の両方を `true` にします。

| 変数 | デフォルト値 | 説明 |
|---|---|---|
| `OPENAPI_ENABLED` | `false` | Dify CLI（`difyctl`）が利用する `/openapi/v1/*` プログラマティック API のマスタースイッチ。<br/>`false` の間はすべての `/openapi/v1/*` エンドポイントが 404 を返し、`difyctl login` を開始できません。`ENABLE_OAUTH_BEARER` と併せて有効にします。 |
| `ENABLE_OAUTH_BEARER` | `false` | `/openapi/v1/*` の OAuth ベアラー認証を有効にし、デバイスフローのログイン承認も制御します。<br/>`OPENAPI_ENABLED=true` でもこれが `false` だと、`difyctl login` は承認時に 503 で失敗します。CLI には両方の `true` が必要です。 |
| `OPENAPI_KNOWN_CLIENT_IDS` | `difyctl` | デバイスフローのログインを開始できる OAuth クライアント ID（カンマ区切り）。<br/>ID を追加すればコード変更なしで他のクライアントを許可できます。`difyctl` を外すと CLI ログインが `unsupported_client` で失敗します。 |
| `OPENAPI_CORS_ALLOW_ORIGINS` | （空） | `/openapi/v1/*` をクロスオリジンで呼び出せるブラウザオリジン（カンマ区切り）。<br/>空の場合はすべて拒否します。標準デプロイでは影響がなく、CLI などブラウザ以外のクライアントは CORS の対象外です。 |
| `OPENAPI_RATE_LIMIT_PER_TOKEN` | `60` | OAuth ベアラートークンごとの `/openapi/v1/*` に対する 1 分あたりのリクエスト上限。Redis 経由でレプリカ間に共有されます。<br/>超過すると 429 と `Retry-After` ヘッダーを返します。`0` で無効化できます。 |
| `DEVICE_FLOW_APPROVE_RATE_LIMIT_PER_HOUR` | `10` | 1 つのブラウザセッションが 1 時間に承認・拒否できるデバイスフローログインの回数。<br/>超えると承認が 429 を返し、ウィンドウのリセット待ちになります。`0` は指定できません。 |
...
| `SENTRY_TRACES_SAMPLE_RATE` | `1.0` | `API_SENTRY_TRACES_SAMPLE_RATE` の直接名で、`docker/envs/core-services/` 配下の設定ファイルが使用します。<br/>Compose 構成では `API_SENTRY_*` の値が優先されます。Sentry DSN が空の間は無効です。 |
| `SENTRY_PROFILES_SAMPLE_RATE` | `1.0` | `API_SENTRY_PROFILES_SAMPLE_RATE` の直接名で、用途は上と同じです。<br/>プロファイリングはトレース対象に選ばれたリクエストからのみ採取されるため、実際の量はおおよそ両レートの積になります。 |
...
| 変数 | デフォルト値 | 説明 |
|---|---|---|
| `ENABLE_LEARN_APP` | `true` | Learn Dify セクションの表示を切り替えます。対象は探索ページのチュートリアルテンプレート、スタジオにアプリがないときの入門パネル、ヘルプメニューの表示切り替えです。<br/>`false` で 3 か所とも非表示になります。 |

#### LLMOps トレース配信

アプリ実行のトレースは、バックグラウンドタスクが LLMOps プロバイダーへ配信します。対象のプロバイダーは Langfuse、LangSmith、Arize/Phoenix などです。以下の変数は、一時的な失敗時のリトライを制御します。

| 変数 | デフォルト値 | 説明 |
|---|---|---|
| `OPS_TRACE_RETRYABLE_DISPATCH_MAX_RETRIES` | `60` | 一時的な失敗後にトレース配信をリトライする最大回数。超えるとそのトレースは破棄されます。<br/>現在この経路に入るのは、Arize/Phoenix でネストしたワークフローをトレースする場合のみです。子トレースが親のコンテキストより先に届くと発生します。`0` は指定できません。 |
| `OPS_TRACE_RETRYABLE_DISPATCH_DELAY_SECONDS` | `5` | リトライの間隔（秒）。許容ウィンドウは間隔 × 最大回数（デフォルトで約 5 分）です。<br/>小さくするとネストしたトレースが早く届く一方、リトライ回数を早く使い切ります。`0` は指定できません。 |
...
| 変数 | デフォルト値 | 説明 |
|---|---|---|
| `NEW_USER_DEFAULT_PLUGIN_IDS` | （空） | 新しいワークスペースの作成時に自動インストールされるマーケットプレイスのプラグイン ID（カンマ区切り）。 |
| `NEW_USER_DEFAULT_MODELS` | （空） | 新しいワークスペースに設定されるデフォルトモデル。`model_type:provider:model` 形式のカンマ区切り。 |

#### ワークフロー生成

| 変数 | デフォルト値 | 説明 |
|---|---|---|
| `WORKFLOW_GENERATION_TIMEOUT_MS` | `180000` | `/create` または `/refine` によるアプリ生成 1 回を UI が待つ時間。web コンテナが読み込む。 |
| `WORKFLOW_GENERATOR_NODE_BUILDER_MAX_WORKERS` | `6` | `/create`／`/refine` の生成リクエスト 1 回あたりの、ノード構成を組み立てる LLM 呼び出しの最大並列数。 |
...
Go to Anything コマンドパレットの `/create` と `/refine` コマンドを有効にします。これらは自然言語の説明から Workflow や Chatflow アプリを生成・修正します。`false` に設定すると非表示になります。
...
デフォルト値：（`.env.example` に開発用キーを設定済み。本番環境では必ず置き換えてください）

Agent バックエンドが内部トークンの鍵を導出するためのルートシークレットです。サンドボックス内の Agent がバックエンドを呼び出す際のトークンもここから導出されます。

デフォルトは公開された開発用キーのため、デプロイはそのまま起動しますが、このキーを知っていれば誰でもトークンを偽造できます。本番環境で使用する前に置き換えてください。

値はパディングなしの base64url テキストで、デコードするとちょうど 32 バイトになる必要があります。値が不正な場合、Agent バックエンドは起動しません。次のコマンドで生成できます：
...
API が Agent バックエンドにアクセスするためのアドレスです。Docker Compose ではデフォルトのままにします。

その他の変数は、同梱のサービスを設定するか、既存の Compose 値から導出されるため、通常は変更不要です。

| 変数 | デフォルト値 | 説明 |
|---|---|---|
| `DIFY_AGENT_REDIS_URL` | （空。`REDIS_PASSWORD` から導出） | Agent バックエンドが実行状態を保存する Redis 接続。Compose のデフォルトは同梱の `redis` のデータベース 2。 |
...
| `AGENT_BACKEND_RUN_TIMEOUT_SECONDS` | `1200` | Agent 実行 1 回のイベントストリーム全体の制限時間。超過した実行は失敗する。 |
...
| `DIFY_AGENT_PLUGIN_DAEMON_API_KEY` | （空。`PLUGIN_DAEMON_KEY` から導出） | プラグインデーモンの認証情報。 |
| `DIFY_AGENT_INNER_API_URL` | （空。`PLUGIN_DIFY_INNER_API_URL` から導出） | Agent バックエンドの内部呼び出し用 Dify API アドレス。 |
| `DIFY_AGENT_INNER_API_KEY` | （空。`PLUGIN_DIFY_INNER_API_KEY` から導出） | API サービスの `INNER_API_KEY_FOR_PLUGIN` と一致させる（`INNER_API_KEY` ではない）。 |
| `DIFY_AGENT_SHELLCTL_ENTRYPOINT` | `http://local_sandbox:5004` | 同梱のサンドボックスサービスのアドレス。 |
| `DIFY_AGENT_SHELLCTL_AUTH_TOKEN` | （空） | Agent バックエンドからサンドボックスへの呼び出しを保護する任意の Bearer トークン。Compose は同じ値を両サービスに渡す。 |
| `DIFY_AGENT_STUB_API_BASE_URL` | `http://agent_backend:5050/agent-stub` | サンドボックス内の Agent が Agent バックエンドを呼び出すためのアドレス。 |
...
| `PLUGIN_REMOTE_INSTALL_HOST` | `localhost` | コンソールがプラグイン開発者に提示するリモートデバッグ接続先。表示専用で、デーモンの待ち受けには影響しません。<br/>開発者が到達できるホスト名を設定してください（Compose 構成では `EXPOSE_PLUGIN_DEBUGGING_HOST`）。デフォルトの `localhost` は Docker ホスト上で開発する場合のみ有効です。 |
| `PLUGIN_REMOTE_INSTALL_PORT` | `5003` | 提示されるデバッグ接続先のポート部分。<br/>デーモンが公開するデバッグポート（`EXPOSE_PLUGIN_DEBUGGING_PORT`）と一致している必要があります。Compose 構成では同じ変数から両方が設定されます。 |
...
| `MARKETPLACE_URL` | （空。Compose 構成では `https://marketplace.dify.ai` にフォールバック） | Marketplace サイトへ遷移するリンクのベース URL。プラグイン詳細ページ、「もっと見る」リンク、公開ガイドが対象です。<br/>`MARKETPLACE_API_URL` とは別物で、そちらはコンソールとプラグインデーモンが Marketplace データの取得に使います。 |
