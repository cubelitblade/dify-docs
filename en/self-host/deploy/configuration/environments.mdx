...
The `/openapi/v1/*` surface is the user-scoped programmatic API that the [Dify CLI](/en/cli/quick-start) (`difyctl`) and other clients authenticate against with OAuth bearer tokens. It ships disabled: set both `OPENAPI_ENABLED` and `ENABLE_OAUTH_BEARER` to `true` to use the CLI against your deployment.

| Variable | Default | Description |
|---|---|---|
| `OPENAPI_ENABLED` | `false` | Master switch for the `/openapi/v1/*` programmatic API used by the Dify CLI (`difyctl`).<br/>When `false`, every `/openapi/v1/*` endpoint returns 404 and `difyctl login` cannot start. Enable together with `ENABLE_OAUTH_BEARER`. |
| `ENABLE_OAUTH_BEARER` | `false` | Binds the OAuth bearer authenticator for `/openapi/v1/*` and gates device-flow login approval.<br/>With `OPENAPI_ENABLED=true` but this `false`, `difyctl login` fails with 503 at the approval step. Both must be `true` for the CLI to work. |
| `OPENAPI_KNOWN_CLIENT_IDS` | `difyctl` | Comma-separated OAuth client IDs allowed to start a device-flow login.<br/>Add IDs to admit other clients without code changes; removing `difyctl` breaks CLI login with `unsupported_client`. |
| `OPENAPI_CORS_ALLOW_ORIGINS` | (empty) | Browser origins allowed to call `/openapi/v1/*` cross-origin, comma-separated.<br/>Empty allows none, which is fine for the standard deployment and irrelevant to non-browser clients like the CLI. |
| `OPENAPI_RATE_LIMIT_PER_TOKEN` | `60` | Requests per minute per OAuth bearer token across `/openapi/v1/*`, shared across API replicas via Redis.<br/>Exceeding it returns 429 with a `Retry-After` header. `0` disables the limit. |
| `DEVICE_FLOW_APPROVE_RATE_LIMIT_PER_HOUR` | `10` | Device-flow login approvals or denials one browser session can perform per hour.<br/>Beyond that, approvals return 429 until the window passes. Cannot be `0`. |
...
| `SENTRY_TRACES_SAMPLE_RATE` | `1.0` | Direct name for `API_SENTRY_TRACES_SAMPLE_RATE`, used by the config files under `docker/envs/core-services/`.<br/>In the Compose stack the `API_SENTRY_*` value wins. No effect while the Sentry DSN is empty. |
| `SENTRY_PROFILES_SAMPLE_RATE` | `1.0` | Direct name for `API_SENTRY_PROFILES_SAMPLE_RATE`, used by the same config files.<br/>Profiling only samples requests already selected by the traces rate, so effective volume is traces rate × profiles rate. |
...
| Variable | Default | Description |
|---|---|---|
| `OPS_TRACE_RETRYABLE_DISPATCH_MAX_RETRIES` | `60` | How many times a trace delivery is retried after a transient failure before the trace is dropped.<br/>Today only Arize/Phoenix tracing of nested workflows hits this path (a child trace arriving before its parent's context). Cannot be `0`. |
| `OPS_TRACE_RETRYABLE_DISPATCH_DELAY_SECONDS` | `5` | Seconds between those retries; the total tolerance window is delay × max retries (about 5 minutes at defaults).<br/>Lower delivers nested traces sooner but exhausts the retry budget faster. Cannot be `0`. |
...
Maximum size in MB for a skill package (`.zip` or `.skill`) uploaded to an agent as an embedded skill. Larger packages are rejected with a size error.

Only agent-side uploads read this limit; importing a skill into the skill library has its own fixed caps (50 files, 512 KB each, 5 MB per package) that this variable doesn't change. If you raise it above 100, also raise `NGINX_CLIENT_MAX_BODY_SIZE` to match; otherwise Nginx rejects the upload with a 413 error.
...
| `PLUGIN_REMOTE_INSTALL_HOST` | `localhost` | Address the console shows plugin developers for remote plugin debugging. Display-only: the daemon's listener is unaffected.<br/>Set it (via `EXPOSE_PLUGIN_DEBUGGING_HOST` in the Compose stack) to a host developers can reach; `localhost` only works when developing on the Docker host itself. |
| `PLUGIN_REMOTE_INSTALL_PORT` | `5003` | Port half of the advertised debugging address.<br/>Must match the published daemon debugging port (`EXPOSE_PLUGIN_DEBUGGING_PORT`); the Compose stack keeps the two in sync from that one knob. |
...
| `MARKETPLACE_URL` | (empty; the Compose stack falls back to `https://marketplace.dify.ai`) | Base URL for links that send users out to the Marketplace website (plugin detail pages, "more" links, the publishing guide).<br/>Distinct from `MARKETPLACE_API_URL`, which the console and plugin daemon use to fetch Marketplace data. |
...
Creator Center (`creators.dify.ai`) is the submission portal where users upload Dify apps as templates. Submitted templates are reviewed and, once approved, listed on Dify Marketplace. The console's **Publish to Marketplace** action exports the app's DSL and uploads it to Creator Center on the user's behalf. See [Publish Apps to Marketplace](/en/self-host/use-dify/publish/publish-to-marketplace) for the user-facing flow.
