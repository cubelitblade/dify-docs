...
For example, a support agent's prompt might set its role as a helper that answers any customer question, while its `refund-requests` skill only handles refund tasks: look up the order, refund only within 30 days, and end with a help-center link.

Click **Add** and choose where the skill comes from:

- Choose **From skill library** to add a [*library skill*](/en/self-host/use-dify/build/skills) shared workspace-wide, up to 20. The agent follows the skill's latest published version.
...
<Info>
To allow packages larger than 50 MB, raise [`UPLOAD_SKILL_FILE_SIZE_LIMIT`](/en/self-host/deploy/configuration/environments#upload_skill_file_size_limit) in your environment variables.
</Info>

<Note>
Referencing a Dify tool, file, or knowledge base in a skill doesn't use it. To use one, add it to the agent.
</Note>
