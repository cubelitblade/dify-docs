---
title: 环境变量
description: Dify 自部署使用的所有环境变量参考
---
...
| `API_WEBSOCKET_WORKER_AMOUNT` | `1` | 仅适用于 Docker Compose。专用 `api_websocket` 容器中服务实时协作的 Gunicorn 工作进程数。协作事件通过 Redis 在各工作进程间同步，因此在编辑流量较大、单个工作进程占满一个 CPU 核心时，可放心调高此值。 |
...
| `API_WEBSOCKET_WORKER_CONNECTIONS` | `1000` | 仅适用于 Docker Compose。每个 `api_websocket` 工作进程可同时接受的 WebSocket 连接数上限。每个打开的工作流编辑器标签页占用一个连接，因此默认值下每个工作进程最多支持 1000 个并发编辑会话；如有更多需求，可调高此值或增加工作进程。 |
...
默认值：`worker`（worker 服务 env 文件的预置值；完全未设置该变量时，容器运行 API 服务）

决定 `dify-api` 镜像在容器启动时运行的进程。三个 env 示例文件分别为各自服务固定取值：`worker.env.example` 为 `worker`，`api.env.example` 为 `api`，`worker-beat.env.example` 为 `beat`。可选值：

- `api`（空值或无法识别的值同样生效）：API 服务
- `worker`：Celery 任务消费进程
- `beat`：定时任务调度进程
- `job`：执行一条 Flask CLI 命令后退出
- `migration`：配合 `MIGRATION_ENABLED=true`，完成数据库迁移后退出

自带的 Compose 文件已为每个服务固定正确的值，只有 Kubernetes 等自定义部署才需要设置。配置错误会让容器运行错误的进程：没有 worker 时，索引和邮件任务会一直排队。
...
| `REDIS_KEEPALIVE` | `true` | 为 Redis 连接启用 TCP keepalive，以便及时发现被防火墙或负载均衡器静默断开的空闲连接并重新建立，而非在下次使用时才失败。 |
| `REDIS_KEEPALIVE_IDLE` | `30` | 连接空闲多少秒后发送第一个 TCP keepalive 探测包（`TCP_KEEPIDLE`）。在 `REDIS_KEEPALIVE=true` 时生效。 |
| `REDIS_KEEPALIVE_INTERVAL` | `10` | 两次 TCP keepalive 探测包之间的间隔秒数（`TCP_KEEPINTVL`）。在 `REDIS_KEEPALIVE=true` 时生效。 |
| `REDIS_KEEPALIVE_COUNT` | `10` | 连接被判定为失效前，允许未收到确认的探测包次数（`TCP_KEEPCNT`）。在 `REDIS_KEEPALIVE=true` 时生效。 |
...
`/openapi/v1/*` 是面向用户的编程 API，[Dify CLI](/zh/cli/quick-start)（`difyctl`）等客户端通过 OAuth Bearer Token 访问。该接口默认关闭：将 `OPENAPI_ENABLED` 和 `ENABLE_OAUTH_BEARER` 都设为 `true` 后，CLI 才能连接你的部署。

| 变量 | 默认值 | 说明 |
|---|---|---|
| `OPENAPI_ENABLED` | `false` | `/openapi/v1/*` 编程 API 的总开关，Dify CLI（`difyctl`）依赖该接口。<br/>为 `false` 时所有 `/openapi/v1/*` 端点返回 404，`difyctl login` 无法开始。需与 `ENABLE_OAUTH_BEARER` 一同开启。 |
| `ENABLE_OAUTH_BEARER` | `false` | 为 `/openapi/v1/*` 绑定 OAuth Bearer 鉴权器，并控制设备流登录的批准端点。<br/>若只开启 `OPENAPI_ENABLED` 而此项为 `false`，`difyctl login` 会在批准环节收到 503。两者都为 `true` 时 CLI 才可用。 |
| `OPENAPI_KNOWN_CLIENT_IDS` | `difyctl` | 允许发起设备流登录的 OAuth 客户端 ID，逗号分隔。<br/>新增 ID 即可接入其他客户端，无需改代码；移除 `difyctl` 会让 CLI 登录报 `unsupported_client`。 |
| `OPENAPI_CORS_ALLOW_ORIGINS` | （空） | 允许跨域调用 `/openapi/v1/*` 的浏览器来源，逗号分隔。<br/>为空表示全部拒绝：标准部署下没有影响，CLI 等非浏览器客户端也不受 CORS 限制。 |
| `OPENAPI_RATE_LIMIT_PER_TOKEN` | `60` | 每个 OAuth Bearer Token 在 `/openapi/v1/*` 上的每分钟请求上限，经 Redis 在多副本间共享。<br/>超限返回 429 并附 `Retry-After` 头。设为 `0` 可关闭限流。 |
| `DEVICE_FLOW_APPROVE_RATE_LIMIT_PER_HOUR` | `10` | 单个浏览器会话每小时可批准或拒绝的设备流登录次数。<br/>超出后批准请求返回 429，需等待窗口重置。不能设为 `0`。 |
...
| `SENTRY_TRACES_SAMPLE_RATE` | `1.0` | `API_SENTRY_TRACES_SAMPLE_RATE` 的直接变量名，供 `docker/envs/core-services/` 下的配置文件使用。<br/>Compose 部署中以 `API_SENTRY_*` 的值为准。Sentry DSN 为空时不生效。 |
| `SENTRY_PROFILES_SAMPLE_RATE` | `1.0` | `API_SENTRY_PROFILES_SAMPLE_RATE` 的直接变量名，用途同上。<br/>性能分析只在已被追踪采样的请求中采样，实际采样量约为两个比例的乘积。 |
...
| 变量 | 默认值 | 说明 |
|---|---|---|
| `ENABLE_LEARN_APP` | `true` | 显示 Learn Dify 版块：探索页的教程模板、工作室中还没有应用时的入门面板，以及帮助菜单中的显示开关。<br/>设为 `false` 可同时隐藏三处。 |

#### LLMOps 追踪投递

应用运行的追踪数据由后台任务投递给所配置的 LLMOps 供应商（Langfuse、LangSmith、Arize/Phoenix 等）。以下变量控制遇到瞬时失败后的重试行为。

| 变量 | 默认值 | 说明 |
|---|---|---|
| `OPS_TRACE_RETRYABLE_DISPATCH_MAX_RETRIES` | `60` | 追踪投递遇到瞬时失败后的最大重试次数，超出后该条追踪数据被丢弃。<br/>目前只有 Arize/Phoenix 追踪嵌套工作流时会触发（子追踪先于父级上下文到达）。不能设为 `0`。 |
| `OPS_TRACE_RETRYABLE_DISPATCH_DELAY_SECONDS` | `5` | 两次重试的间隔秒数；总容忍窗口约为间隔 × 最大次数（默认约 5 分钟）。<br/>调低可让嵌套追踪更快送达，但重试次数消耗更快。不能设为 `0`。 |
...
| 变量 | 默认值 | 说明 |
|---|---|---|
| `NEW_USER_DEFAULT_PLUGIN_IDS` | （空） | 创建新工作空间时自动安装的市场插件 ID，逗号分隔。 |
| `NEW_USER_DEFAULT_MODELS` | （空） | 为新工作空间预设的默认模型，逗号分隔，格式为 `model_type:provider:model`。 |

#### 工作流生成

| 变量 | 默认值 | 说明 |
|---|---|---|
| `WORKFLOW_GENERATION_TIMEOUT_MS` | `180000` | 界面等待一次 `/create` 或 `/refine` 应用生成的时长。由 web 容器读取。 |
| `WORKFLOW_GENERATOR_NODE_BUILDER_MAX_WORKERS` | `6` | 单次 `/create` 或 `/refine` 生成请求中，并发构建节点配置的 LLM 调用上限。 |
...
启用 Go to Anything 命令面板中的 `/create` 和 `/refine` 命令，这两个命令可根据自然语言描述生成和修改 Workflow 与 Chatflow 应用。设为 `false` 即可隐藏它们。
...
默认值：（`.env.example` 中预填了开发密钥，生产环境必须替换）

Agent 后端的根密钥，用于派生各类内部令牌的密钥，其中包括沙箱内 Agent 回调后端时使用的令牌。

默认值是一个公开的开发密钥，部署可开箱即用；但任何知道该密钥的人都能伪造这些令牌，生产环境使用前务必替换。

值必须是不带填充的 base64url 文本，且解码后恰好为 32 字节；值格式错误时，Agent 后端将拒绝启动。
...
在 Web 界面中显示新 Agent：主导航中的 **Agents** 项，以及工作流节点选择器中的新 Agent 节点。
...
API 访问 Agent 后端所用的地址。在 Docker Compose 中保持默认值即可。

其余变量用于配置打包的服务，或从现有的 Compose 配置中推导，通常无需改动。

| 变量 | 默认值 | 说明 |
|---|---|---|
| `DIFY_AGENT_REDIS_URL` | （空；由 `REDIS_PASSWORD` 推导） | Agent 后端存储运行状态的 Redis 连接；Compose 默认指向打包的 `redis` 的 2 号数据库。 |
...
| `AGENT_BACKEND_RUN_TIMEOUT_SECONDS` | `1200` | 单次 Agent 运行事件流的总时限，超时后运行失败。 |
...
| `DIFY_AGENT_PLUGIN_DAEMON_API_KEY` | （空；由 `PLUGIN_DAEMON_KEY` 推导） | 插件守护进程的凭证。 |
| `DIFY_AGENT_INNER_API_URL` | （空；由 `PLUGIN_DIFY_INNER_API_URL` 推导） | Agent 后端内部调用的 Dify API 地址。 |
| `DIFY_AGENT_INNER_API_KEY` | （空；由 `PLUGIN_DIFY_INNER_API_KEY` 推导） | 必须与 API 服务的 `INNER_API_KEY_FOR_PLUGIN` 一致，而非 `INNER_API_KEY`。 |
| `DIFY_AGENT_SHELLCTL_ENTRYPOINT` | `http://local_sandbox:5004` | 打包的沙箱服务地址。 |
| `DIFY_AGENT_SHELLCTL_AUTH_TOKEN` | （空） | 可选的 Bearer 令牌，用于保护 Agent 后端对沙箱的调用；Compose 会把同一值传给两个服务。 |
| `DIFY_AGENT_STUB_API_BASE_URL` | `http://agent_backend:5050/agent-stub` | 沙箱内 Agent 回调 Agent 后端的地址。 |
...
| `POSITION_PROVIDER_PINS` | （空） | 将特定模型供应商固定在顶部。示例：`openai,anthropic`。 |
| `POSITION_PROVIDER_INCLUDES` | （空） | 仅显示列出的模型供应商。如未设置，所有供应商可用。 |
| `POSITION_PROVIDER_EXCLUDES` | （空） | 隐藏特定模型供应商（固定的供应商不受影响）。 |
...
| `PLUGIN_REMOTE_INSTALL_HOST` | `localhost` | 控制台向插件开发者展示的远程调试地址。仅用于展示，不影响插件守护进程实际监听的地址。<br/>需设置为开发者可访问的主机名（Compose 部署中通过 `EXPOSE_PLUGIN_DEBUGGING_HOST` 设置）；默认的 `localhost` 只适用于在 Docker 宿主机上开发的场景。 |
| `PLUGIN_REMOTE_INSTALL_PORT` | `5003` | 远程调试地址中的端口部分。<br/>必须与守护进程对外发布的调试端口（`EXPOSE_PLUGIN_DEBUGGING_PORT`）一致；Compose 部署中两者由同一变量保持同步。 |
...
| `MARKETPLACE_URL` | （空；Compose 部署回退到 `https://marketplace.dify.ai`） | 跳转到 Marketplace 网站的链接基础 URL（插件详情页、「更多」链接、发布指南）。<br/>与 `MARKETPLACE_API_URL` 不同：后者供控制台和插件守护进程获取 Marketplace 数据。 |
